...
function renderTabs(){ const modeProducts = getModeCatalog(); const categoryTabs = document.querySelector('#category-tabs'); const filterContainer = document.querySelector('#catalog-filters'); if (!modeProducts.length) { categoryTabs.innerHTML = '<p class="empty">Próximamente: este catálogo estará disponible.</p>'; filterContainer.innerHTML = ''; return; } const categoryOrder = catalogModes[activeCatalogMode]?.categories || []; const groups = categoryOrder
    .filter(categoryId => modeProducts.some(product => product.category === categoryId))
    .map(categoryId => [categoryId, catalog.find(product => product.category === categoryId)?.categoryLabel || categoryId]); const tabs = [['all','Todo'], ...groups]; const filterMarkup = showClothingFilters() ? `
...
const openQuoteForm = () => { if (!cart.length) return; quoteDialog.showModal(); requestAnimationFrame(() => quoteDialog.scrollIntoView({ block: 'center' })); };
document.querySelectorAll('[data-open-quote]').forEach(button => button.addEventListener('click', openQuote)); document.querySelector('[data-close-quote]').addEventListener('click', closeQuote); overlay.addEventListener('click', closeQuote); document.querySelector('#open-form').addEventListener('click', openQuoteForm); document.querySelector('.modal-close').addEventListener('click', () => quoteDialog.close());
