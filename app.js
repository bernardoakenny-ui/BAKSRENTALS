...
function renderTabs(){ const modeProducts = getModeCatalog(); if (!modeProducts.length) { document.querySelector('#category-tabs').innerHTML = '<p class="empty">Próximamente: este catálogo estará disponible.</p>'; return; } const categoryOrder = catalogModes[activeCatalogMode]?.categories || []; const groups = categoryOrder
    ...
    .map(categoryId => [categoryId, catalog.find(product => product.category === categoryId)?.categoryLabel || categoryId]); const tabs = [['all','Todo'], ...groups]; document.querySelector('#category-tabs').innerHTML = tabs.map(([id,label]) => `<button class="${activeCategory === id ? 'active' : ''}" data-category="${id}">${formatCategoryLabel(label)}</button>`).join(''); document.querySelectorAll('[data-category]').forEach(button => button.addEventListener('click', () => { activeCategory = button.dataset.category; renderTabs(); renderProducts(); })); }
